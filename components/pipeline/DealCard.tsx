...
export default function DealCard({ deal, onEdit, onDelete, onMove, isDragging }: DealCardProps) {
  ...
  const getPriorityColor = (value: number, probability: number) => {
    if (value >= 100000 && probability >= 70) return 'bg-red-500' // High value, high probability
    if (value >= 50000 && probability >= 50) return 'bg-orange-500' // Medium-high
    if (value >= 25000 || probability >= 70) return 'bg-yellow-500' // Medium
    return 'bg-green-500' // Low priority/value
  }
  ...
  const getHealthStatus = () => {
    ...
    if (closeDate && closeDate < now && deal.stage_name !== 'Closed Won') {
      ...
      return { status: 'stalled', color: 'text-orange-600 bg-orange-100', icon: AlertCircle, label: 'Stalled' }
    } else if (deal.probability >= 80) {
      return { status: 'hot', color: 'text-green-600 bg-green-100', icon: TrendingUp, label: 'Hot' }
    } else if (closeDate) {
      const daysToClose = Math.floor((closeDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
      if (daysToClose <= 7 && daysToClose > 0) {
        return { status: 'closing', color: 'text-blue-600 bg-blue-100', icon: Target, label: 'Closing Soon' }
      }
    }
  ...
  const getCloseDateStatus = () => {
    ...
    if (diffDays < 0) {
      ...
      return { text: 'Closes today', color: 'text-orange-600 bg-orange-100' }
    } else if (diffDays === 1) {
      return { text: 'Closes tomorrow', color: 'text-yellow-600 bg-yellow-100' }
    } else if (diffDays <= 7) {
      return { text: `Closes in ${diffDays} days`, color: 'text-blue-600 bg-blue-100' }
    } else if (diffDays <= 30) {
    ...
    <div className={`bg-white border rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-200 ${
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {/* Priority icon for high-value deals */}
          {(deal.value >= 50000 && deal.probability >= 50) && (
            <PriorityIcon className={`w-4 h-4 ${
              deal.value >= 100000 && deal.probability >= 70 ? 'text-red-500' : 'text-orange-500'
            }`} />
          )}
        </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t">
        <div className="flex items-center gap-3 text-xs">
          {/* Close date */}
          {closeDateStatus && (
            <span className={`px-2 py-1 rounded-full ${closeDateStatus.color}`}>
