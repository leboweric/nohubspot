...
def create_contact(db: Session, contact: ContactCreate) -> Contact:
    ...
    if db_contact.company_id:
        company = get_company(db, db_contact.company_id)
        if company:
...
def update_contact(db: Session, contact_id: int, contact_update: ContactUpdate) -> Optional[Contact]:
    ...
    if old_company_id != new_company_id:
        if old_company_id:
            old_company = get_company(db, old_company_id)
            if old_company and old_company.contact_count > 0:
                old_company.contact_count -= 1
        
        if new_company_id:
            new_company = get_company(db, new_company_id)
            if new_company:
...
def create_email_thread(db: Session, thread: EmailThreadCreate) -> EmailThread:
    db_thread = EmailThread(**thread.dict())
    db.add(db_thread)
    db.commit()
...
def add_email_message(db: Session, message: EmailMessageCreate) -> EmailMessage:
    db_message = EmailMessage(**message.dict())
    db.add(db_message)
    
    # Update thread message count and preview
    thread = db.query(EmailThread).filter(EmailThread.id == message.thread_id).first()
    if thread:
...
def create_attachment(db: Session, attachment: AttachmentCreate) -> Attachment:
    ...
    if db_attachment.company_id:
        company = get_company(db, db_attachment.company_id)
        if company:
...
def get_email_signature(db: Session, user_id: str = "default") -> Optional[EmailSignature]:
    return db.query(EmailSignature).filter(EmailSignature.user_id == user_id).first()
...
def create_or_update_email_signature(
    ...
    existing_signature = get_email_signature(db, user_id)
    ...
    if existing_signature:
        ...
        db_signature = EmailSignature(**signature_data.dict(), user_id=user_id)
        db.add(db_signature)
        db.commit()
...
# Bulk operations
def bulk_create_companies(db: Session, companies: List[CompanyCreate]) -> List[Company]:
    db_companies = []
...
def bulk_create_contacts(db: Session, contacts: List[ContactCreate]) -> List[Contact]:
    db_contacts = []
...
def get_dashboard_stats(db: Session) -> DashboardStats:
    ...
    total_email_threads = db.query(EmailThread).count()
