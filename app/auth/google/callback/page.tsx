...
function GoogleCallbackContent() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        const result = await googleIntegrationAPI.handleCallback(code, state || undefined)
        
        if (result.success) {
          setStatus('success')
          setMessage(`Connected to Google Workspace as ${result.email}`)
          
          // Close popup and notify parent window
          if (window.opener) {
            window.opener.postMessage({ type: 'google-connected', email: result.email }, '*')
